...
struct library {
    char book_name[100]; // Increased the size to accommodate longer book names
    char author[100];    // Increased the size to accommodate longer author names
    int pages;
    float price;
};
        ...
        switch (input) {
            case 1: // Add book information
                if (count < 100) {
                    ...
                    scanf(" %[^\n]s", lib[count].book_name); // Read with spaces
                    printf("Enter author name: ");
                    scanf(" %[^\n]s", lib[count].author);    // Read with spaces
                    printf("Enter pages: ");
            ...
            case 3: // List all books by a given author
                if (count == 0) {
                    ...
                    printf("Enter author name: ");
                    scanf(" %[^\n]s", author_name); // Read with spaces
                    int found = 0;
                    printf("Books by %s:\n", author_name);
                    for (int i = 0; i < count; i++) {
